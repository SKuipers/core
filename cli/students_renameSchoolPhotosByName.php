...
if (php_sapi_name() != 'cli') { echo __($guid, 'This script cannot be run from a browser, only via CLI.');
    ...
    $photoPathStudentsOutput = $_SESSION[$guid]['absolutePath'] .'/uploads/photosStudentsName';
    ...
    foreach ($photos as $filename) {
        ...
        try {
            ...
            $sql = "SELECT username, surname, firstName, preferredName FROM gibbonPerson WHERE surname=:surname AND :firstname LIKE CONCAT('%', preferredName, '%') AND gibbonRoleIDPrimary=003 AND username LIKE '2%'";
