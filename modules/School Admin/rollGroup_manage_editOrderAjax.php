...
$_POST['address'] = '/modules/School Admin/rollGroup_manage.php';
...
if (isActionAccessible($guid, $connection2, '/modules/School Admin/rollGroup_manage.php') == false) {
    ...
    if (empty($order) || empty($data['gibbonSchoolYearID'])) {
        ...
        $count = $_POST['index'] ?? 1;
        foreach ($order as $gibbonRollGroupID) {
            $updated = $rollGroupGateway->update($gibbonRollGroupID, ['sequenceNumber' => $count]);
            $count++;
        }
    }
}
