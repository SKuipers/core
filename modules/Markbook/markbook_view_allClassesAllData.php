...
use Gibbon\Module\Markbook\MarkbookColumn;
...
// Get class variable
$gibbonCourseClassID = $_GET['gibbonCourseClassID'] ?? $session->get('markbookClass') ?? '';
...
for ($i = 0; $i < $markbook->getColumnCountThisPage(); ++$i) {
    ...
    if ($column->displayAttainment()) {
        ...
        if (empty($attainmentScale)) {
            $rowScale = $gradeScaleGateway->getByID($column->getData('gibbonScaleIDAttainment'));
            if (!empty($rowScale)) {
    ...
    if ($column->displayEffort()) {
        $rowScale = $gradeScaleGateway->getByID($column->getData('gibbonScaleIDEffort'));
        if (!empty($rowScale)) {
...
$count = 0;
$totals = [];
$resultStudents = $courseEnrolmentGateway->selectClassStudentEnrolment($gibbonCourseClassID, $studentOrderBy);
...
if ($resultStudents->rowCount() < 1) {
    ...
    while ($rowStudents = $resultStudents->fetch()) {
        ...
        for ($i = 0; $i < $markbook->getColumnCountThisPage(); ++$i) {
            ...
            if (!empty($rowEntry)) {
                $entryData = array_merge($entryData, $rowEntry);
