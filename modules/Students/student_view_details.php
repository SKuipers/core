            ...
            if (isActionAccessible($guid, $connection2, '/modules/Students/student_view_details.php', 'View Student Profile_brief') and $skipBrief == false) {
                ...
                if ($result->rowCount() != 1) {
                    ...
                    if ($subpage == 'Overview') {
                        ...
                        echo '</p>';
                        try {
                            $dataDetail = array('gibbonPersonID' => $gibbonPersonID, 'gibbonYearGroupID' => $row['gibbonYearGroupID'], 'gibbonSchoolYearID' => $_SESSION[$guid]['gibbonSchoolYearID'], 'gibbonRollGroupID' => $row['gibbonRollGroupID']);
                            $sqlDetail = "
                            (SELECT DISTINCT teacher.surname, teacher.preferredName, teacher.email, gibbonCourseClass.gibbonCourseClassID as entryID, gibbonCourse.name as entryName, CONCAT(gibbonCourse.nameShort, '.', gibbonCourseClass.nameShort) as entryNameShort, 5 as listOrder FROM gibbonPerson AS teacher JOIN gibbonCourseClassPerson AS teacherClass ON (teacherClass.gibbonPersonID=teacher.gibbonPersonID)  JOIN gibbonCourseClassPerson AS studentClass ON (studentClass.gibbonCourseClassID=teacherClass.gibbonCourseClassID) JOIN gibbonPerson AS student ON (studentClass.gibbonPersonID=student.gibbonPersonID) JOIN gibbonCourseClass ON (studentClass.gibbonCourseClassID=gibbonCourseClass.gibbonCourseClassID) JOIN gibbonCourse ON (gibbonCourseClass.gibbonCourseID=gibbonCourse.gibbonCourseID) WHERE teacher.status='Full' AND teacherClass.role='Teacher' AND studentClass.role='Student' AND student.gibbonPersonID=:gibbonPersonID AND gibbonCourse.gibbonSchoolYearID=(SELECT gibbonSchoolYearID FROM gibbonSchoolYear WHERE status='Current') ORDER BY teacher.preferredName, teacher.surname, teacher.email )
                                UNION
                                (SELECT DISTINCT surname, preferredName, email, ''  as entryID, 'Head of Year' as entryName, 'HOY' as entryNameShort, 1 as listOrder FROM gibbonPerson JOIN gibbonYearGroup ON (gibbonYearGroup.gibbonPersonIDHOY=gibbonPersonID) WHERE status='Full' AND gibbonYearGroupID=:gibbonYearGroupID)
                                UNION
                                (SELECT DISTINCT surname, preferredName, email, ''  as entryID, 'Educational Assistant' as entryName, 'EA' as entryNameShort, 2 as olistOrderrder
                                    FROM gibbonPerson
                                ...
                                UNION
                                (SELECT DISTINCT surname, preferredName, email, ''  as entryID, 'Educational Assistant' as entryName, 'EA' as entryNameShort, 3 as listOrder
                                    FROM gibbonPerson
                        ...
                        if ($resultDetail->rowCount() < 1) {
                            ...
                            while ($rowDetail = $resultDetail->fetch()) {
                                echo '<li><span style="min-width:320px;display:inline-block;">';
                                    ...
                                    if (!empty($rowDetail['entryID'])) {
                                        echo "<a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/Departments/department_course_class.php&gibbonCourseClassID=".$rowDetail['entryID']."' title='".$rowDetail['entryNameShort']."'>";
                                        echo __($rowDetail['entryName']).'</a>';
                                    } else {
                                        echo __($rowDetail['entryName']);
                                    }
                                echo '</li>';
                            }
                            echo '</ul>';
