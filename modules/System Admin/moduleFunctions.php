...
function getCurrentVersion($guid, $connection2, $version)
{
    ...
    $output .= 'fetch("https://gibbonedu.org/services/version/version.php?callback=fnsuccesscallback", {';
    $output .= 'method: "GET", mode: "cors"';
    $output .= '})';
    $output .= '.then(response => response.text())';
    $output .= '.then(text => {';
    $output .= 'const jsonMatch = text.match(/fnsuccesscallback\\((.*)\\)/);';
    $output .= 'if (jsonMatch && jsonMatch[1]) { return JSON.parse(jsonMatch[1]); }';
    $output .= 'throw new Error("Invalid JSONP response");';
    $output .= '})';
    $output .= '.then(data => {';
    $output .= 'const statusEl = document.getElementById("status");';
    $output .= 'const gibbonCheckEl = document.getElementById("gibbonCheck");';
    $output .= "if (data['version']==='false') {";
    $output .= 'statusEl.setAttribute("class","error");';
    $output .= "statusEl.innerHTML = '".__('Version check failed').".';";
    $output .= '}';
    $output .= 'else {';
    $output .= "if (versionCompare(data['version'], '".$version."') <= 0) {";
    $output .= "if (gibbonCheckEl) gibbonCheckEl.innerHTML = '<span class=\"tag rounded-full success\">".__('OK')."</span>';";
    $output .= 'statusEl.setAttribute("class","success");';
    $output .= "statusEl.innerHTML = '".sprintf(__('Version check successful. Your Gibbon installation is up to date at %1$s.'), $version).' '.sprintf(__('If you have recently updated your system files, please check that your database is up to date in %1$sUpdates%2$s.'), "<a href=\'".$session->get('absoluteURL')."/index.php?q=/modules/System Admin/update.php\'>", '</a>')."';";
    $output .= '}';
    $output .= 'else {';
    $output .= 'statusEl.setAttribute("class","warning");';
    $output .= "if (gibbonCheckEl) gibbonCheckEl.innerHTML = '<span class=\"tag rounded-full warning\">".__('Update')."</span>';";
    $output .= "statusEl.innerHTML = '".sprintf(__('Version check successful. Your Gibbon installation is out of date. Please visit %1$s to download the latest version.'), "<a target=\"blank\" href=\'https://gibbonedu.org/download\'>the Gibbon download page</a>")."';";
    $output .= '}';
    $output .= '}';
    $output .= '})';
    $output .= '.catch(error => {';
    $output .= 'const statusEl = document.getElementById("status");';
    $output .= 'const gibbonCheckEl = document.getElementById("gibbonCheck");';
    $output .= "if (gibbonCheckEl) gibbonCheckEl.innerHTML = '<span class=\"tag rounded-full warning\">".__('Error')."</span>';";
    $output .= 'statusEl.setAttribute("class","error");';
    $output .= "statusEl.innerHTML = '".__('Version check failed').".';";
    $output .= 'console.error("Version check error:", error);';
    $output .= '});';
    $output .= '});';
    $output .= '</script>';
