    ...
    protected function getElement()
    {
        ...
        if (!empty($this->attachments)) {
                ...
                if (!empty($attachmentPath)) {
                    ...
                    if ($this->canDelete) {
                        ...
                        if (!empty($this->deleteAction)) {
                            ...
                            $output .= "<div title='".__('Delete')."' class='inline-button text-gray-600' onclick='if(confirm(\"".__('Are you sure you want to delete this record?').' '.__('Changes will be saved when you submit this form.')."\")) { document.getElementById(\"".$attachmentNameEscaped."\").value = \"\"; document.getElementById(\"".$idEscaped."\").style.display = \"block\"; document.getElementById(\"".$idEscaped."\").disabled = false; this.parentElement.remove(); };'>";
                            $output .= icon('solid', 'delete', 'size-6 sm:size-5');
                            $output .= '</div>';
                        }
                    }
            ...
            if ($this->getAttribute('multiple') == true) {
                ...
                $output .= '<a onClick="const el = document.querySelector(\'#'.$idEscaped.'\'); el.style.display = \'block\'; el.disabled = false; ">'.__('Upload File').'</a>';
                $output .= '</div>';
            }
