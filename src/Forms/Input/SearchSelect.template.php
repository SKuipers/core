<div id="<?= $id ?>SearchSelect" x-data="{
        ...
        <!-- trigger button  -->
        <button type="button" class="<?= $class; ?> <?= $groupClass; ?> inner-input inline-flex w-full overflow-hidden items-center justify-between min-w-16 h-[2.375rem] bg-white dark:bg-gray-800 border border-outline dark:border-gray-600 py-2 px-3 text-gray-900 dark:text-gray-100  placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:border-blue-500 dark:focus:border-blue-400 focus-within:border-blue-500 dark:focus-within:border-blue-400 focus:ring-1 focus:ring-inset focus:ring-blue-500 dark:focus:ring-blue-400 sm:text-sm sm:leading-5 transition  " role="combobox" aria-controls="<?= $id ?>List" aria-haspopup="<?= $id ?>List listbox" x-on:click="toggleSelect(!isOpen)" x-on:keydown.down.prevent="openedWithKeyboard = true" x-on:keydown.enter.prevent="openedWithKeyboard = true" x-on:keydown.space.prevent="openedWithKeyboard = true" x-bind:aria-expanded="isOpen || openedWithKeyboard" x-bind:aria-label="selectedOption ? selectedOption.label : '<?= __($placeholder); ?>'" x-ref="searchSelect" >
            ...
            <!-- Chevron  -->
            <svg x-cloak x-show="selectedOption == null" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-gray-500 dark:text-gray-400" aria-hidden="true">
                <path fill-rule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd"/>
            </svg>

            <svg x-cloak x-show="selectedOption != null" x-on:click.stop="clearSelectedOption()" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-gray-500 dark:text-gray-400 hover:text-red-700 dark:hover:text-red-400 transition-colors" aria-hidden="true">
                ...
                <template x-for="(group, groupIndex) in options" x-bind:key="group.label">
                    ...
                    <ul x-from-template class="list-none flex flex-col m-0 p-1">
                        <template x-if="group.label != ''">
                            <li x-from-template x-text="group.label" class="px-3 py-1 text-sm dark:text-gray-300  font-semibold"></li>
                        </template>

                        <template x-for="(item, index) in group.options" x-bind:key="item.value">

                            <li x-from-template class="combobox-option inline-flex justify-between rounded px-3 py-1 text-sm dark:text-gray-100  hover:bg-blue-500 hover:text-white focus:bg-blue-500 focus:text-white cursor-pointer whitespace-nowrap" role="option" x-on:click="setSelectedOption(item)" x-on:keydown.enter="setSelectedOption(item)" x-bind:id="'option-' + index" tabindex="0" :class="{'bg-gray-300 dark:bg-gray-700 text-gray-900 dark:text-gray-100 hover:text-white' : selectedOption == item , 'hover:text-white': selectedOption != item, 'pl-8' : group.label != '' }">
